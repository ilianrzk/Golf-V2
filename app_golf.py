...
with st.sidebar.expander("Ouvrir l'assistant", expanded=False):
    ...
    if not df_analysis.empty:
        # On cherche les coups 'Jeu Long'
        df_caddie = df_analysis[df_analysis['type_coup'] == 'Jeu Long']
        
        # On essaie de filtrer par Lie, sinon on prend tout
        df_lie = df_caddie[df_caddie['lie'] == cad_lie]
        if len(df_lie) < 10: 
            df_lie = df_caddie # Fallback si pas assez de données sur ce lie
            
        # Calcul des moyennes par club
...
# GÉNÉRATEUR DE DONNÉES (V30 - Dates variées pour tester le filtre)
if st.sidebar.button("Générer Données V30 (Test)"):
    new_data = []
    # Dates étalées sur 6 mois
...
# ==================================================
# ONGLET 3 : SIMULATEUR (NOUVEAU)
# ==================================================
with tab_simu:
    st.header("🎮 Simulateur de Parcours")
    st.caption("Jouez des trous virtuels au practice !")
    
    sim = st.session_state['sim_state']
    
    if not sim['active']:
        if st.button("⛳ Commencer un Trou"):
            # Générer un trou aléatoire
            par = np.random.choice([3, 4, 5])
            dist = {3: 130, 4: 360, 5: 480}[par] + np.random.randint(-20, 20)
            st.session_state['sim_state'] = {
                'active': True, 'hole_num': sim['hole_num'], 'par': par, 
                'total_dist': dist, 'remaining': dist, 'shots': 0, 'history': []
            }
            st.rerun()
    else:
        st.info(f"### Trou {sim['hole_num']} | Par {sim['par']} | {sim['total_dist']}m")
        st.metric("Distance Restante", f"{sim['remaining']}m")
        
        # Choix du club intelligent
        rec_club = "Driver"
        ...
        st.write(f"Coups joués : {sim['shots']}")
        
        # Saisie Simu
        col_sim1, col_sim2 = st.columns(2)
        with col_sim1:
            s_club = st.selectbox("Club", CLUBS_ORDER, index=CLUBS_ORDER.index(rec_club) if rec_club in CLUBS_ORDER else 0
